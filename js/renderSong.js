...
export const renderSong = (data) => {
  ...
  try {
    ...
    const createArtistSection = (titleText, artists) => {
      ...
      const artistsIndividualClass = titleText
        .replace(/\s+/g, '-')
        .toLowerCase();
    ...
    if (
      ...
      receivedSong.producer_artists.length > 0
    ) {
    ...
    let writerArtists;
    if (receivedSong.writer_artists && receivedSong.writer_artists.length > 0) {
    ...
    setTimeout(() => {
      if (workingArea.contains(loader)) {
        workingArea.removeChild(loader);
      }
